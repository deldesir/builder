...
function executeClientScriptRestricted(
	...
	const handler = {
		get(target: Element, prop: string | symbol) {
			// IIAB storefront escape hatches: storefront client scripts need to unwrap
			// proxies via `__target` and reach a few real DOM helpers (wrapped on return).
	...
	const context = {
		...
		props,
		component_data: componentData,
		// IIAB storefront: expose a minimal `window` (getComputedStyle), $clamp,
		// Node.contains and a stubbed location so storefront client scripts run.
		window: {
		...
		$clamp: (el: any, options: any) => window.$clamp(el?.__target || el, options),
		dispatch: () => {},
		Node: {
			prototype: {
				contains: (node: any) =>
					Node.prototype.contains.call(proxiedThis?.__target || proxiedThis, node?.__target || node),
			},
		},
		location: {
			href: "",
